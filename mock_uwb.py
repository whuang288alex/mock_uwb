    ...
    while run:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False
                break
            # randomly generate a new tag position if the enter key is pressed
            elif event.type == pygame.KEYDOWN:
                if event.key == pygame.K_RETURN:
                    tag = [random.randint(200, 1160), random.randint(200, 770)]
                    new_point = True
                    
        # draw anchor spots, outer frame and tag
        draw_dot(anchor1)
        draw_dot(anchor2)
        draw_dot(anchor3)
        draw_dot(anchor4)
        ...
        p1, p2 = get_cirlce_collision(r1, r2, anchor1, anchor2)
        p1, p2 = extend_line(p1, p2)
       
        p3, p4 = get_cirlce_collision(r2, r3, anchor2, anchor3)
        p3, p4 = extend_line(p3, p4)
        
        p5, p6 = get_cirlce_collision(r3, r1, anchor3, anchor1)
        p5, p6 = extend_line(p5, p6)
